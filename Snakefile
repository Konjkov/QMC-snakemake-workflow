...
def vmc_energy(method, basis, molecule, *path_spec):
    ...
    try:
        with open(os.path.join(method, basis, molecule, *path_spec, 'out'), 'r') as vmc_out:
            # we are only interested in the last occurrence
...
def vmc_variance(method, basis, molecule, *path_spec):
    ...
    try:
        with open(os.path.join(method, basis, molecule, *path_spec, 'out'), 'r') as vmc_opt_out:
            # we are only interested in the last occurrence
...
def dmc_energy(method, basis, molecule, *path_spec):
    """Get DMC energy.
...
def dmc_stderr(method, basis, molecule, *path_spec):
...
def dmc_ncorr(method, basis, molecule, *path_spec):
...
def dmc_stats_nstep(method, basis, molecule, *path_spec):
...
rule VMC_DMC_BF_INPUT:
    ...
    run:
        neu, ned = get_up_down(wildcards.method, wildcards.basis, wildcards.molecule)
        hf, _ = vmc_energy(wildcards.method, wildcards.basis, wildcards.molecule, *('VMC', '10000000'))
        vmc, _ = vmc_energy(wildcards.method, wildcards.basis, wildcards.molecule, *('VMC_OPT_BF', wildcards.jastrow_opt_method, 'casl', wildcards.jastrow_rank + '__' + wildcards.backflow_rank, '1000000_9'))
        dtdmc = 1.0/(get_max_Z(wildcards.molecule)**2 * 3.0 * params.dt_relative_step)
        nstep = params.magic_const*(hf - vmc)/(int(wildcards.nconfig)*dtdmc*params.stderr*params.stderr)
        nstep=max(50000, int(round(nstep, -3)))
        with open(output[0], 'w') as f:
